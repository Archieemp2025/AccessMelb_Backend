...
def load_toilets():
    ...
    # Quick sanity check — nearest accessible toilets to NGV
    ngv = "ST_SetSRID(ST_MakePoint(144.9690, -37.8224), 4326)::geography"
    cur.execute(f"""
        SELECT name, wheelchair_accessible,
               ROUND(ST_Distance(location::geography, {ngv})::numeric, 1) AS distance_m
        FROM public_toilet
        WHERE ST_DWithin(location::geography, {ngv}, 500)
          AND wheelchair_accessible = 'yes'
        ORDER BY distance_m
        LIMIT 5
    """)
    ...
    nearby = cur.fetchall()
    if nearby:
        print(f"\nNearest accessible toilets to NGV (within 500m):")
        for name, _, dist in nearby:
            print(f"  {dist}m — {name}")
